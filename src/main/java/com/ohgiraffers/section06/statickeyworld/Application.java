...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        *  따라서 여러 새의 인스턴스가 같은 공간을 공유할 목적으로 필드에 static 키워드를 사용한다.
        * */
        ...
        // static 메소드 호출
        StaticMethodTest.staticMethod();        // 권장함, 클래스명.메소드명 으로 호출

        smt.staticMethod();     // 이렇게 쓸수도 있지만 권장하지 않음, 메모리가 낭비가 됨
        /* 주의사항
        * static 메소드도 이렇게 호출은 할 수 있고, 실제 사용할 수도 있다.
        * 하지만 static메소드의 경우에는 인스턴스를 생성하지 않고 호출하는 방식으로 사용해야 한다.
        * static method는 정적 영역에 두고 인스턴스를 생성하지 않고 호출할 목적으로 만들기 때문데
